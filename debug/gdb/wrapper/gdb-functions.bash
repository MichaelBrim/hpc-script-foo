...
function jobutil_debug_daemon {
    # USAGE: jobutil_debug_daemon executable-path "daemon args"
    if [[ $# -ne 2 ]]; then
        echo >&2 "$jutil_err USAGE: jobutil_debug_daemon exe-path \"daemon args ...\""
        return 1
    fi
    ...
    echo >&2 "$jutil_info DAEMON-DEBUG: on $my_host - exe=$my_exe, args=\"$my_args\", logdir=$my_logdir"
...
EOS
    ...
    gdb_exec="gdb"
    [[ -d $my_exe_dir/.libs && -x $my_exe_dir/.libs/$my_exe_base ]] && gdb_exec="libtool --mode=execute $gdb_exec"
    echo >&2 "$jutil_info DAEMON-DEBUG: on $my_host - running: $gdb_exec $gdb_args $my_exe > ${my_log_base}.gdb-log"
    $gdb_exec $gdb_args $my_exe &> ${my_log_base}.gdb-log & 
}
declare -fx jobutil_debug_daemon

function jobutil_debug_app {
    # USAGE: jobutil_debug_app application-exe-path "app args"
    if [[ $# -ne 2 ]]; then
        echo >&2 "$jutil_err USAGE: jobutil_debug_app app-exe-path \"app args ...\""
        return 1
    fi
    ...
    echo >&2 "$jutil_info APP-DEBUG: on $my_host - exe=$my_exe, args=\"$my_args\", logdir=$my_logdir"
...
EOS
    ...
    gdb_exec="gdb"
    [[ -d $my_exe_dir/.libs && -x $my_exe_dir/.libs/$my_exe_base ]] && gdb_exec="libtool --mode=execute $gdb_exec"
    echo >&2 "$jutil_info APP-DEBUG: on $my_host - running: $gdb_exec $gdb_args $my_exe | tee ${my_log_base}.gdb-log"
    $gdb_exec $gdb_args $my_exe 2>&1 | tee ${my_log_base}.gdb-log
}
declare -fx jobutil_debug_app
